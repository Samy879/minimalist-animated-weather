...
plasmoidName=$(grep '"Id"' "$ROOT_DIR/metadata.json" | cut -d'"' -f4)
...
potFile="$TRANSLATE_DIR/template.pot"
...
poFiles=`find "$TRANSLATE_DIR" -maxdepth 1 -name '*.po' | sort`
...
rm -rf "$ROOT_DIR/contents/locale"/*/LC_MESSAGES/*.mo

echo "[build] Compilation pour le domaine : $projectName"

for po in $poFiles; do
    catLocale=`basename ${po%.*}`
    installPath="$ROOT_DIR/contents/locale/${catLocale}/LC_MESSAGES/${projectName}.mo"
